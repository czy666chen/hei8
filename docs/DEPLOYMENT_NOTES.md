...
> 本文档基于生产部署实际操作持续整理；最近一次部署为 2026-08-21 v6.0.0（实时房间独立奇招牌与 UI 收敛，见第 9 节变更记录）。
...
- 正式发布 v6.0.0：追分 / 中八实时房间加入独立奇招牌、按观察者过滤手牌、抽取 / 使用 / 跳过 / 下一轮手牌数与重发事件；中八本地对局支持双方不同起始手牌并按局重发。
- 实时房间 UI 对齐普通对局计分板和奇招牌布局，补齐席位选择认领、新一轮手牌可见性与白天主题样式。
- 发布前验证：单元测试 80 项、Worker 集成测试 64 项、房间端到端测试 8 项、发布目录 ESLint 与 production build 全部通过；D1 无待应用迁移，3 个生产 secrets 均存在且未修改。
- 原地部署至既有 Worker `taiqiu-qizhao-cards`，Version `69d3e6c9-bf87-4dc7-b54e-3d2272f75fa0`，D1 继续使用 `hei8-r3-production-v2`。
- Cloudflare 部署记录确认新版本 100% 生效；本机访问 `*.workers.dev` 时被 Windows Schannel `SEC_E_NO_CREDENTIALS` 阻断（HTTP 000），因此未能从当前网络完成 `/api/health`、首页 200 与未登录 401 的直连复核。
