...
- 生产 Worker `hei8-r3-production` 已部署，三个生产 Secret 已独立配置；生产 D1 迁移、健康检查、匿名权限、错误邀请码和发布包能力冒烟均已通过。真实邀请码账号灰度与生产备份恢复演练仍待人工完成。球友/认领属于后续独立功能切片，不阻塞本次账号与单主写同步候选。
...
5. [x] 部署并验收预览候选；[x] 部署生产候选并完成自动化冒烟；[ ] 使用真实邀请码进行内部账号灰度。
