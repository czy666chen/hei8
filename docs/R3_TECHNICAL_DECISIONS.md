...
- 生产 Worker 尚未创建，且生产三个 Secret 尚未配置；这是切换生产前唯一外部配置门槛。球友/认领属于后续独立功能切片，不阻塞本次账号与单主写同步候选。
...
### 阶段 7：导出、删除和发布验收（预览完成，待生产切换）

1. [x] 实现 JSON 数据导出和密码确认账号删除。
2. [x] 实现已撤销会话、同步回执和审计定时清理。
3. [ ] 完成生产备份恢复演练（生产 Worker 与 Secret 配置后执行）。
4. [x] 运行单元测试、授权集成测试、桌面 E2E 和移动端 E2E。
5. [x] 部署并验收预览候选；[ ] 配置生产 Secret 后进行内部账号灰度。
