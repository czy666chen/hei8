...
- 共 87 个测试通过，其中 18 个为真实 Worker+D1 集成测试；lint、preview/production 构建、迁移校验和仓库/构建产物秘密扫描通过。
- 预览 Worker 已部署，三个真实 Secret、远程 D1 迁移、健康检查、账号、幂等迁移和 A/B 授权冒烟均已验收；冒烟账号及关联数据已清理。
- 统一业务授权、业务 API 和本地迁移已完成；账号前端、离线同步、球友/认领、导出删除、生产 Secret 与发布验收仍待后续阶段完成。
...
5. [ ] 生产发布前单独配置 production 的三个真实 Secret。
6. [x] 增加仓库和构建产物秘密扫描。

验收状态：本地游客页面和 `/api/health` 同源可用；三套 D1 结构已校验；秘密扫描通过。预览 Worker、真实 Secret、远程迁移与 API 冒烟已完成，生产配置保留到发布阶段。
