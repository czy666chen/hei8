...
### 阶段 1：Cloudflare 同源运行时与 D1 骨架

1. 配置 Worker Static Assets、`/api/*` 优先路由和 D1 binding。
2. 建立 local、preview、production 三套隔离配置。
3. 建立可重复执行的 Drizzle/D1 migrations。
4. 配置 `REGISTRATION_INVITE_CODE`、`PASSWORD_HMAC_KEY`、`SESSION_HMAC_KEY` Secrets。
5. 增加仓库和构建产物秘密扫描。

验收：游客页面和 `/api/health` 同源可用；空库迁移可重复执行；秘密不进入 Git 或构建产物。

### 阶段 2：轻量账号闭环

1. 实现用户名规范化和 HMAC-SHA256 工具。
2. 实现固定邀请码注册。
3. 实现登录、会话恢复和退出。
4. 实现修改密码和管理员重置密码命令。
5. 实现资料、昵称和公开编号。
...
验收：错误邀请码无法注册；正确邀请码可重复注册不同账号；D1 不含明文密码和原始会话令牌；更改昵称不影响用户 ID。

### 阶段 3：业务表与授权

1. 建立球友、预设、牌组、对局、事件、认领和同步表。
2. 建立统一 `requireSession`、`requireMatchRead`、`requireMatchWriteLease` 授权函数。
3. 使用账号 A、账号 B 和匿名用户覆盖所有越权路径。
4. 为用户历史、球友、对局详情和同步查询验证索引使用情况。

验收：未授权读取和修改全部失败；所有用户身份由服务端会话推导。

### 阶段 4：本地迁移

1. 抽离当前 `localStorage` 访问为版本化存储适配器。
2. 实现迁移预览、用户确认、本机备份和校验和。
3. 实现幂等批量上传和服务端校验。
4. 覆盖成功、断网、重复提交、部分失败和取消测试。
...
1. 实现 JSON 数据导出和账号删除。
2. 实现已撤销/无效会话、同步回执和审计定时清理。
3. 完成免费额度监控和备份恢复演练。
4. 运行单元测试、授权集成测试、桌面 E2E 和移动端 E2E。
5. 先使用内部测试账号灰度，再开放固定邀请码注册。
